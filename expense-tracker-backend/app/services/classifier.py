...
class NaiveBayesClassifier:
    def __init__(self):
        # Initialize necessary dictionaries and variables
        self.word_probs = defaultdict(lambda: defaultdict(float))  # Word probabilities for each class and category
        self.class_counts = defaultdict(int)  # Counts of each class (income, expense)
        self.category_counts = defaultdict(lambda: defaultdict(int))  # Counts of each category per class
        self.total_docs = 0  # Total number of documents (transactions)
        self.vocab = set()  # Set to hold unique words in the training data
        self.classes = ['income', 'expense']  # Possible transaction types
        self.categories = {
            'income': ['salary', 'investment', 'gift', 'freelance', 'rental', 'income'],
            'expense': ['food', 'transport', 'bills', 'entertainment', 'housing', 'healthcare', 'education']
        }
    ...
    def preprocess(self, text):
        """
        Preprocesses the text by lowercasing it and tokenizing into words.
        """
        text = text.lower()
        words = re.findall(r'\w+', text)
        return words
    ...
    def train(self, transactions):
        """
        Trains the classifier using the provided transactions.
        Each transaction is a tuple of (description, class, category).
        """
        for text, cls, category in transactions:
            if cls not in self.classes or category not in self.categories[cls]:
                continue
            # Update class and category counts
            self.class_counts[cls] += 1
            self.category_counts[cls][category] += 1
            self.total_docs += 1
            # Process words in the description
            words = self.preprocess(text)
            for word in words:
                self.vocab.add(word)
                self.word_probs[cls][word] += 1
        ...
        # Calculate word probabilities for each class and category
        vocab_size = len(self.vocab)
        for cls in self.classes:
            for word in self.vocab:
    ...
    def classify(self, text):
        """
        Classifies a text description and returns the predicted class and category.
        """
        words = self.preprocess(text)
        class_scores = {}

        # Compute class scores based on prior probability and likelihood
        for cls in self.classes:
            prior = math.log(self.class_counts[cls] / self.total_docs if self.total_docs > 0 else 1)
            likelihood = 0
            for word in words:
        ...
        # Compute category scores for the predicted class
        for category in self.categories[predicted_class]:
            prior = math.log(self.category_counts[predicted_class][category] / 
                             self.class_counts[predicted_class] if self.class_counts[predicted_class] > 0 else 1)
            likelihood = 0
            for word in words:
                if word in self.vocab:
                    likelihood += math.log(self.word_probs[f"{predicted_class}_{category}"][word])
    ...
    def load_training_data(self, file_path):
        """
        Loads the training data from a CSV file and trains the classifier.
        """
        with open(file_path, 'r', encoding='utf-8') as f:
            reader = csv.DictReader(f)
            transactions = [(row['description'], row['class'], row['category']) for row in reader]
    ...
    def predict_category(self, description, transaction_type):
        """
        Predicts the category for a given description and transaction type.
        Adds thresholding to classify as 'uncategorized' if the prediction confidence is low.
        """
        ...
        # Calculate the scores for each category within the specified transaction type
        for category in self.categories[transaction_type]:
            prior = math.log(self.category_counts[transaction_type][category] / 
                             self.class_counts[transaction_type] if self.class_counts[transaction_type] > 0 else 1)
            likelihood = 0
            for word in words:
                if word in self.vocab:
        ...
        # Normalize the category scores and calculate the probabilities
        ...
        # Apply threshold for low-confidence predictions
        predicted_category = max(category_scores, key=category_scores.get)
        if probabilities[predicted_category] < 0.10:  # Thresholding for weak predictions
